...
export default function PricingPage() {
  return (
    <Layout title="料金 | 楽々省エネ計算" description="公式BEIワークフローと住宅の公式検証に対する料金ページです。">
      <div className="pt-24 pb-20">

        {/* ── ヘッダー ── */}
        <div className="max-w-3xl mx-auto px-6 text-center mb-16">
          <p className="text-xs font-semibold text-orange-600 uppercase tracking-widest mb-3">Pricing</p>
        ...
        <div className="max-w-4xl mx-auto px-6 mb-16">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
            ...
            <div className="rounded-2xl border border-slate-200 bg-white p-7 flex flex-col">
              ...
              <p className="text-xs text-slate-400 mb-6">カード登録不要・期限なし</p>
              <Link href="/register" className="block w-full text-center text-sm font-semibold border border-slate-200 hover:border-slate-300 text-slate-700 py-3 rounded-xl transition-colors mb-6">
                無料で始める
              </Link>
              <ul className="space-y-3 flex-1">
                {freeFeatures.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm text-slate-500">
                    <FaCheckCircle className="text-slate-300 text-xs mt-1 flex-shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
            ...
            <div className="rounded-2xl border-2 border-orange-300 bg-white p-7 relative shadow-[0_8px_30px_rgba(234,88,12,.08)] flex flex-col">
              <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-xs font-bold px-4 py-1.5 rounded-full tracking-wider">
                おすすめ
              </div>
              <p className="text-xs font-bold text-orange-600 uppercase tracking-widest mb-5">Monthly</p>
              <div className="mb-2">
              ...
              <p className="text-xs text-slate-400 mb-6">いつでも解約OK・自動更新</p>
              <Link href="/register" className="block w-full text-center text-sm font-semibold bg-orange-500 hover:bg-orange-600 text-white py-3 rounded-xl transition-colors mb-6">
                月額プランを開始 →
              </Link>
              <div className="bg-orange-50 border border-orange-100 rounded-xl px-4 py-3 mb-5">
                <p className="text-sm font-bold text-orange-800 mb-1">全案件で使い放題</p>
                <p className="text-xs text-orange-600">何案件やっても月額固定。2案件以上なら確実にお得。</p>
              </div>
              <ul className="space-y-3 flex-1">
                {paidFeatures.map((f) => (
                  <li key={f} className="flex items-start gap-2.5 text-sm text-slate-600">
                    <FaCheckCircle className="text-orange-500 text-xs mt-1 flex-shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
            ...
            {/* 1案件パス */}
            <div className="rounded-2xl border border-slate-200 bg-white p-7 flex flex-col">
              <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-5">Per project</p>
              <div className="mb-2">
              ...
              <p className="text-xs text-slate-400 mb-6">1案件だけ使いたいとき</p>
              <Link href="/register" className="block w-full text-center text-sm font-semibold border border-slate-200 hover:border-slate-300 text-slate-700 py-3 rounded-xl transition-colors mb-6">
                1案件パスを購入
              </Link>
              <div className="bg-slate-50 border border-slate-100 rounded-xl px-4 py-3 mb-5">
                <p className="text-sm font-bold text-slate-700 mb-1">買い切り・30日間有効</p>
                <p className="text-xs text-slate-400">選んだ1プロジェクト専用。終わったらそのまま終了。</p>
              </div>
              <ul className="space-y-3 flex-1">
              ...
              <p className="mt-auto pt-4 text-xs text-slate-400 text-center">
        ...
        <div className="max-w-3xl mx-auto px-6 mb-16">
          <h2 className="text-xl font-bold text-slate-900 mb-6">機能比較</h2>
        ...
        {/* ── AI建築サークル ── */}
        <div className="max-w-3xl mx-auto px-6 mb-16">
          <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6 flex flex-col md:flex-row items-start gap-5">
            <div className="w-10 h-10 rounded-xl bg-slate-200 flex items-center justify-center flex-shrink-0">
              <span className="text-slate-500 text-sm font-bold">AI</span>
            </div>
            <div>
              <h3 className="text-sm font-bold text-slate-900 mb-1">AI建築サークル会員はそのまま利用可能</h3>
        ...
        <div className="max-w-2xl mx-auto px-6 mb-16">
          ...
          <div className="space-y-2">
            {[
              ...
              <details key={f.q} className="group bg-white rounded-xl border border-slate-100 overflow-hidden hover:border-slate-200 transition-colors">
                <summary className="cursor-pointer px-5 py-4 text-sm font-semibold text-slate-800 flex items-center justify-between select-none">
                  {f.q}
                ...
                <div className="px-5 pb-4 text-sm text-slate-400 leading-relaxed">{f.a}</div>
              </details>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-2xl mx-auto px-6">
          <div className="text-center bg-slate-900 rounded-3xl p-10 md:p-14">
            ...
            <Link href="/register" className="inline-flex items-center justify-center text-sm font-semibold text-slate-900 bg-white hover:bg-slate-100 px-8 py-3.5 rounded-xl transition-colors">
              無料で始める →
            </Link>
          </div>
        </div>
