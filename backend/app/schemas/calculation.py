...
class BuildingInput(BaseModel):
    ...
    has_central_heat_source: Optional[bool] = Field(False, description="中央熱源の有無")
...
class EnvelopePartInput(BaseModel):
    ...
    eta_value: Optional[float] = Field(None, description="日射熱取得率 (η値)")


class EnvelopeInput(BaseModel):
    parts: List[EnvelopePartInput] = Field(..., description="外皮部位リスト")
...
class EnvelopeResult(BaseModel):
    ...
    eta_a_value: Optional[float] = Field(None, description="冷房期の平均日射熱取得率 (ηA値)")
...
class PrimaryEnergyResult(BaseModel):
    ...
    # 用途別標準一次エネルギー量（E_Si）。存在すればフロントの部位BEIに使用される。
    standard_energy_by_use: Optional[Dict[str, float]] = Field(None, description="用途別標準一次エネルギー量(MJ)")
