...
async def upload_excel_get_report(file: UploadFile = File(..., description="公式入力シート (.xlsx/.xlsm)")):
    ...
    if not file.filename.endswith((".xlsx", ".xlsm")):
        logger.warning("Invalid file extension for upload: %s", file.filename)
    ...
    try:
        ...
        if len(excel_bytes) > MAX_UPLOAD_SIZE:
            logger.warning("File size exceeds limit: %.2f MB > 10 MB", file_size_mb)
            raise HTTPException(
                status_code=413,
                detail=f"ファイルサイズが上限（10MB）を超えています。({len(excel_bytes) // 1024 // 1024}MB)",
            )

        logger.info("File size validated: %.2f MB, processing report generation", file_size_mb)
        pdf_bytes = await _run_official_with_timeout(get_official_report_from_excel, excel_bytes)
        logger.info("Successfully generated report PDF from uploaded Excel")
        ...
        logger.error("Official API timeout during report generation: %s", str(e))
        raise HTTPException(
            status_code=504,
            detail=f"公式API応答がタイムアウトしました。時間をおいて再実行してください。({str(e)})",
        )
        ...
        if detail == SMALLMODEL_UPLOAD_UNSUPPORTED_MESSAGE:
            logger.warning("SMALLMODEL upload unsupported")
            raise HTTPException(status_code=400, detail=detail)
        logger.warning("Excel input validation error: %s", detail)
        raise HTTPException(status_code=400, detail=f"Excel入力に問題があります: {detail}")
    except Exception as e:
        logger.exception("Failed to generate report from uploaded Excel")
        raise HTTPException(status_code=500, detail=f"公式レポート生成に失敗しました: {str(e)}")
...
async def upload_excel_get_compute(file: UploadFile = File(..., description="公式入力シート (.xlsx/.xlsm)")):
    ...
    try:
        ...
        logger.info("File size validated: %.2f MB, processing computation", file_size_mb)
        result = await _run_official_with_timeout(get_official_compute_from_excel, excel_bytes)
        logger.info("Successfully executed computation from uploaded Excel")
        return result
    except OfficialAPITimeoutError as e:
        logger.error("Official API timeout during computation: %s", str(e))
        raise HTTPException(
            status_code=504,
            detail=f"公式API応答がタイムアウトしました。時間をおいて再実行してください。({str(e)})",
        )
        ...
        logger.exception("Failed to execute computation from uploaded Excel")
        raise HTTPException(status_code=500, detail=f"公式計算に失敗しました: {str(e)}")
